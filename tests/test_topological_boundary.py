...
def test_boundary_regular_extracts_dimension_zero_empty_boundary() -> None:
...
def test_boundary_regular_extracts_path_endpoints_and_cycle_empty_boundary() -> None:
...
def test_boundary_regular_extracts_disk_rim_with_complete_unsigned_closure() -> None:
...
def test_boundary_regular_extracts_single_simplex_boundary_in_dimensions_three_and_four() -> (
    None
):
...
def test_closed_triangle_manifold_is_boundary_regular_with_empty_boundary() -> None:
    ...
    assert isinstance(sphere.topology_state, BoundaryRegular)
    assert all(
        not boundary.mask(degree).any() for degree in range(sphere.dimension + 1)
    )


def test_boundary_regular_rejects_branching_and_nonpure_input() -> None:
    ...
    with pytest.raises(SimplicialError, match="pure"):
        isolated_vertex.boundary_regular()
...
def test_topological_boundary_closure_does_not_overflow_high_incidence() -> None:
    ...
    domain = Complex.from_maximal_simplices(triangles).boundary_regular()
