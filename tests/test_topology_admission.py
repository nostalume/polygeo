...
from polygeo import SimplicialError, SimplicialError as NativeSimplicialError
...
def test_admission_accepts_every_fixed_width_integer_dtype(
    ...
    np.testing.assert_array_equal(native.boundary_matrix(2).toarray(), [[1], [-1], [1]])
...
def test_admission_accepts_strided_and_non_native_endian_input() -> None:
    ...
    np.testing.assert_array_equal(
        NativeComplex.from_maximal_simplices(strided).boundary_matrix(2).toarray(),
        expected,
    )
    np.testing.assert_array_equal(
        NativeComplex.from_maximal_simplices(non_native).boundary_matrix(2).toarray(),
        expected,
    )
...
def test_owner_detaches_from_input_and_distinguishes_equal_admissions() -> None:
    ...
    np.testing.assert_array_equal(first.orientations(2), [1])
...
def test_owner_bound_subset_keeps_refined_owner_alive() -> None:
    ...
    np.testing.assert_array_equal(boundary.mask(1), [True, False, True, True, True])
